...
如果你想在win上使用GCC那么你就要使用[mingw64](https://www.mingw-w64.org/)啦。

> [!info]
> mingw64是有人将GCC 工具链的代码迁移到win上的工具链，现在更推荐使用msys2的UCRT64环境
...
常见的构建工具有[make](https://www.gnu.org/software/make/),[cmake](https://cmake.org/),[xmake](https://xmake.io)等。`make`相当古老且且仅适用于linux平台。`cmake`应该被叫做“构建工具的构建工具”，因为使用`cmake`可以生成多种工具链的配置，而你只需编写一遍`cmake`的配置文件，就可以生成出适用于MSVC+vcpkg或GCC/Clang+make/ninja的配置，之后你就可以使用VS打开生成的工程文件或者在命令行上make就可以编译运行了。

不过我这里主要谈xmake配置、使用，哈哈。**xmake是构建工具，内置一个包管理`xrepo`。**
...
>
> miniforge可商用；anaconda和miniconda由其公司维护，商用收费
...
> [!tip]
> [清华大学镜像站](https://mirrors.tuna.tsinghua.edu.cn/),[中国科学技术大学镜像站](https://mirrors.ustc.edu.cn/),[南京大学镜像站](https://mirror.nju.edu.cn/)，这些镜像站可以方便国外常用软件的下载和换源，以及linux的下载和换源。还有[mirrorz(联合镜像站)](https://mirrorz.org)，它的帮助文档一绝。
>
